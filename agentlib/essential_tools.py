...
class WorkspaceTools:
    ...
    def write_file(self, path: str, content: str, overwrite: bool = True) -> str:
        """
        Writes or creates a text file with the given content. Creates parent directories automatically.
        
        Args:
            path: Relative or absolute path of the file to write.
            content: Text content to write into the file.
            overwrite: If True, overwrites the existing file content. If False, raises an error if the file exists.
        """
        resolved_path = self._resolve_path(path)
